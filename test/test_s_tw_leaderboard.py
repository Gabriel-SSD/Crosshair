...
def test_gcs_initialization_failure(mock_env):
    with patch("silver.tw_leaderboard.utils.GCSClient", side_effect=Exception("GCS fail")):
...
def test_dataframe_creation_failure(mock_env):
    mock_gcs = MagicMock()
    mock_gcs.load_json_gzip.return_value = {"territoryMapId": "O1690000000000", "data": None}
    with patch("silver.tw_leaderboard.utils.GCSClient", return_value=mock_gcs):
        with pytest.raises(SystemExit):
            main()
...
def test_tw_timestamp_failure(mock_env):
    mock_gcs = MagicMock()
    mock_gcs.load_json_gzip.return_value = {"territoryMapId": "INVALID", "data": {}}
    with patch("silver.tw_leaderboard.utils.GCSClient", return_value=mock_gcs):
        with pytest.raises(SystemExit):
            main()
...
def test_bigquery_initialization_failure(mock_env):
    mock_gcs = MagicMock()
    mock_gcs.load_json_gzip.return_value = {"territoryMapId": "O1690000000000", "data": {"totalBanners": []}}
    with patch("silver.tw_leaderboard.utils.GCSClient", return_value=mock_gcs):
        with patch("silver.tw_leaderboard.bigquery.Client", side_effect=Exception("BQ fail")):
            with pytest.raises(SystemExit):
                main()
...
def test_success_flow(mock_env, caplog):
    ...
    mock_gcs = MagicMock()
    mock_gcs.load_json_gzip.return_value = {"territoryMapId": "O1690000000000", "data": data}
