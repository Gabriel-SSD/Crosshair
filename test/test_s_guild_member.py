...
def test_gcs_initialization_failure(mock_env):
    with patch(
        "silver.guild_member.utils.GCSClient",
...
def test_contribution_processing_failure(mock_env):
    guild_data = {
        ...
        "member": [{"playerId": "p1", "memberContribution": [{}]}]
    }
    mock_gcs = MagicMock()
        ...
        "member": [
            {
                "playerId": None,
                "playerName": "A",
                "guildJoinTime": "invalid",
                "memberLevel": "GUILD_LEADER",
            }
        ]
    }
    mock_gcs = MagicMock()
...
def test_bigquery_initialization_failure(mock_env):
    ...
    with patch("silver.guild_member.utils.GCSClient", return_value=mock_gcs):
        with patch(
            "silver.guild_member.bigquery.Client",
        ...
        "member": [
            {
                "playerId": "p1",
                "playerName": "A",
                "guildJoinTime": "1690000000",
                "memberLevel": "GUILD_MEMBER",
            }
        ]
    }
    mock_gcs = MagicMock()
