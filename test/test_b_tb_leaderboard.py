...
from bronze.tb_leaderboard import load_env_var, main
...
def test_api_initialization_failure(mock_env):
    with patch("bronze.tb_leaderboard.API", side_effect=Exception("API fail")):
...
def test_fetch_data_failure(mock_env):
    with patch("bronze.tb_leaderboard.API") as MockAPI:
...
# -------------------------
# Falta campo 'territoryBattleStatus'
# -------------------------
def test_api_missing_territoryBattleStatus(mock_env):
    with patch("bronze.tb_leaderboard.API") as MockAPI:
...
# -------------------------
# Formato inválido de instanceId
# -------------------------
def test_invalid_instance_id_format(mock_env):
    with patch("bronze.tb_leaderboard.API") as MockAPI:
        MockAPI.return_value.fetch_data.return_value = {
            "territoryBattleStatus": {"instanceId": "INVALID"}
        }
        with pytest.raises(SystemExit):
            main()
...
def test_gcs_initialization_failure(mock_env):
    with patch("bronze.tb_leaderboard.API") as MockAPI:
        MockAPI.return_value.fetch_data.return_value = {
            "territoryBattleStatus": {"instanceId": "O1234567890"}
        }
    with patch(
        "bronze.tb_leaderboard.utils.GCSClient",
...
def test_gcs_upload_failure(mock_env):
    ...
    with patch("bronze.tb_leaderboard.utils.GCSClient", return_value=mock_gcs):
        with pytest.raises(SystemExit):
            main()
...
def test_success_flow(mock_env, caplog):
    ...
    mock_api = MagicMock()
    mock_api.fetch_data.return_value = {"territoryBattleStatus": {"instanceId": "O1699999999000"}}

    mock_gcs = MagicMock()
    mock_gcs.upload_json_gzip.return_value = True

    with patch("bronze.tb_leaderboard.API", return_value=mock_api):
        with patch("bronze.tb_leaderboard.utils.GCSClient", return_value=mock_gcs):
            main()
